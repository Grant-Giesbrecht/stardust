...
def _write_value(fh: h5py.Group, key: str, value: Any, show_detail: bool = False) -> None:
    """
    ...
    if isinstance(value, dict):
        ...
        _write_dict(grp, value, show_detail=show_detail)
    ...
    # ---- list of dicts → indexed subgroups -------------------------
        ...
        for i, item in enumerate(value):
            ...
            _write_dict(sub, item, show_detail=show_detail)
...
def _write_dict(fh: h5py.Group, data: dict, show_detail: bool = False) -> None:
    """Iterate a dict and write each entry into the HDF5 group `fh`."""
    for k, v in data.items():
        _write_value(fh, str(k), v, show_detail=show_detail)
...
def dict_to_hdf(root_data: dict,
                save_file: str,
                use_json_backup: bool = False,
                show_detail: bool = False) -> bool:
    """
    Write a (possibly nested) Python dictionary to an HDF5 file.
    ...
    try:
        with h5py.File(save_file, 'w') as fh:
            ...
            _write_dict(fh, root_data, show_detail=show_detail)
        return True
...
def _read_value(node) -> Any:
    ...
    if isinstance(node, h5py.Group):
        if pytype == "list_of_dicts":
            ...
            return [_read_dict(node[k]) for k in sorted(node.keys(), key=int)]
        else:  # plain dict (or unlabelled legacy group)
            return _read_dict(node)
...
def _read_dict(fh: h5py.Group) -> dict:
    return {k: _read_value(fh[k]) for k in fh.keys()}


def hdf_to_dict(filename: str) -> dict | None:
    """
